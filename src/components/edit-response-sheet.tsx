...
export function EditResponseSheet({
  ...
  const hasUnsavedChanges = () => {
    if (!response || !localEditedData) return false;
    return JSON.stringify(response) !== JSON.stringify(localEditedData);
  };
  ...
  return (
    <>
      <Sheet open={isOpen} onOpenChange={(open) => !open && handleCancel()}>
        ...
        >
          <SheetHeader className="px-6 pt-3 border-b">
            <SheetTitle>
              Card {cardName} - Response {response?.response_index}
            </SheetTitle>
          </SheetHeader>
